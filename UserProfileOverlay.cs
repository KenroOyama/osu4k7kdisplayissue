...
namespace osu.Game.Overlays
{
    public partial class UserProfileOverlay : FullscreenOverlay<ProfileHeader>
    {
        ...
        private void userLoadComplete(APIUser loadedUser, IRulesetInfo? userRuleset)
        {
            ...
            // Add osu!mania 4K and 7K performance points to the profile overlay
            if (loadedUser.Statistics?.Mania4KPP > 0)
            {
                Header.AddStatistic(new Statistic
                {
                    Label = "osu!mania 4K PP",
                    Value = loadedUser.Statistics.Mania4KPP.ToString("N0")
                });
            }

            if (loadedUser.Statistics?.Mania7KPP > 0)
            {
                Header.AddStatistic(new Statistic
                {
                    Label = "osu!mania 7K PP",
                    Value = loadedUser.Statistics.Mania7KPP.ToString("N0")
                });
            }
